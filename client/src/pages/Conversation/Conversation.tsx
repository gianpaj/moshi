...
const buildURL = ({
  ...
  url.searchParams.append(
    "text_temperature",
    params.textTemperature.toString(),
  );
  url.searchParams.append("text_topk", params.textTopk.toString());
  url.searchParams.append(
    "audio_temperature",
    params.audioTemperature.toString(),
  );
  ...
  url.searchParams.append(
    "repetition_penalty",
    params.repetitionPenalty.toString(),
  );
...
export const Conversation: FC<ConversationProps> = ({
    ...
    audioRecorder.current.onstop = async () => {
      ...
      if (mimeType.includes("webm")) {
        blob = await fixWebmDuration(
          new Blob(audioChunks.current, { type: mimeType }),
        );
      } else {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div className="main-grid m-auto h-screen max-h-screen w-screen max-w-96 p-4 md:max-w-screen-lg">
            <div className="controls flex items-center justify-center gap-2 text-center">
              {isOver && !isBypass && (
                <Button
                  onClick={() => {
              ...
              <div
                className={`h-4 w-4 rounded-full ${isConnected ? "bg-green-700" : "bg-red-700"}`}
              />
            </div>
            <div className="player relative h-full max-h-full w-full justify-between gap-3 border-2 border-white  md:p-12">
              <ServerAudio
              ...
              <div className="download-links flex flex-col items-center justify-center pt-8 text-sm">
                {audioURL && (
                  <div>
                    <a
                      href={audioURL}
                      download={`moshi audio.${getExtension("audio")}`}
                ...
                {videoURL && (
                  <div>
                    <a
                      href={videoURL}
                      download={`moshi video.${getExtension("video")}`}
                ...
                {videoURL && getExtension("video") === "webm" && (
                  <div>
                    <a
                      href="https://restream.io/tools/webm-to-mp4-converter"
                      target="_blank"
                      rel="noreferrer"
                      className="explain-links block pt-2 text-center italic"
                    >
            ...
            <div
              className="scrollbar player-text border-2 border-white "
          ...
          <div className="m-auto max-w-96 p-4 text-center md:max-w-screen-lg">
            <ServerInfo />
            {!workerAuthId && (
              <ModelParams {...modelParams} isConnected={isConnected} />
            )}
          </div>
